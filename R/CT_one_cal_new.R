...
#' @param cal.datetime Date and time of the calibration
...
#' @export
CT_one_cal_new<-function(data, date, temp, EC.logger, cal.ref, cal.ref.temp, cal.datetime, EC.cal = FALSE) {
  ...
  mean.temp<-data %>%
    dplyr::filter({{date}} == {{cal.datetime}}) %>%
...
mean.ec<-data%>%
  dplyr::filter({{date}} == {{cal.datetime}}) %>%
  dplyr::rename(EC.log = {{EC.logger}}) %>%
  #dplyr::rename(EC.log = contains("EC") | contains("E_C")) %>%
  dplyr::summarise(mean = mean(EC.log)) %>%
  as.numeric()
...
data<-data%>%
  dplyr::mutate(EC_Cal = EC.log + offset.ec,
